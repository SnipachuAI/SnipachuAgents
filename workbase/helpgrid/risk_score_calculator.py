...
def explain_risk_score(price_change_pct: float, liquidity_usd: float, flags_mask: int) -> Dict[str, float]:
    """
    Returns a breakdown of the risk score calculation.
    """
    vol_score = min(abs(price_change_pct) / 10, 1) * 50
    if liquidity_usd > 0:
        liq_score = max(0.0, 30 - (math.log10(liquidity_usd) * 5))
...
def classify_risk_level(score: float) -> str:
    """
    Classify the numerical risk score into levels.
    """
    if score < 25:
        return "Low"
    elif score < 50:
